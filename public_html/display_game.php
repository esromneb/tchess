...
for($x=0;$x<8;$x++) {
  ...
  for($y=0;$y<8;$y++) {
    ...
    $tpl->assign(array(
		       ...
		       "square_html" => ($this_piece = $display->num_to_piecename($piece_array[$x][$y])) ? '<span style="position:absolute; top:0; left:0; z-index:1">'."<img src=\"images/pieces/".$this_piece.".gif\"$img_mouse_over>".'</span>' : "&nbsp;",
		       //		       "onclick" => ($move_array[$x][$y] == "")?"":' onClick="alert('.$move_array[$x][$y].');"',
		       "first_td" => ($y)? "" :" height=\"49\"",
		       ...
		       "green_dot" => ($x==0 || $x==1 || $x==6 || $x==7)?'<span id="'.$x.$y.'" style="z-index:2; position:absolute; top:0; left:0; visibility:hidden;"><img src="images/green_dot2.gif" '."></span>" : "",
		       ));
  }
}
